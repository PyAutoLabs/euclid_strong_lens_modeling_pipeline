...
| The `vis_lp` fits are finished and you want the four Sersic-stage variants on the same lenses | **Sersic variants** | `batch_cpu/submit_sersic_variants` | One lens; `scripts/sersic_lens_model.py` run four times in a row, `--variant=baseline`, `wide_n`, `central_noise`, `sersic_point`, writing to `sersic_lens_model_<variant>/vis`. Sequential rather than a 4N-wide array because all four restore the same `vis_lp` zip, which `restore()` deletes. 4 cores, 14 GB, 18 h, partition `ral`; a failed variant does not stop the rest. |
